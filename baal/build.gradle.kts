...
group = "dev.brella"
version = "2.0.0"
val projectName = "cors-mechanics-baal"
val latestTag = "latest-baal"
...
tasks.create<Dockerfile>("createDockerfile") {
    ...
    copyFile(
        tasks.named<com.github.jengelman.gradle.plugins.shadow.tasks.ShadowJar>("shadowJar")
            .get().archiveFileName.get(), "/app/cors-mechanics.jar"
    )
    copyFile("baal.conf", "/app/application.conf")
    entryPoint("java")
    defaultCommand(
        "-cp",
        "/app/cors-mechanics.jar:/usr/lib/",
        "io.ktor.server.netty.EngineMain",
        "-config=/app/application.conf"
    )
    exposePort(8786)
    runCommand("apk --update --no-cache add curl jq-dev")
    instruction("HEALTHCHECK CMD curl -f http://localhost:6070/health || exit 1")
}
...
tasks.create<Sync>("syncShadowJarArchive") {
    ...
    from(
        tasks.named<com.github.jengelman.gradle.plugins.shadow.tasks.ShadowJar>("shadowJar")
            ...
            .get()
            .asFile,
        rootProject.file("baal.conf")
    )
    into(
        tasks.named<com.bmuschko.gradle.docker.tasks.image.Dockerfile>("createDockerfile")
            .get().destFile.get().asFile.parentFile
    )
}
...
tasks.create<DockerBuildImage>("buildImage") {
    ...
    images.addAll("undermybrella/$projectName:$version", "undermybrella/$projectName:$latestTag")
}
